...
4. Run `run_server.bat` (Windows) or `Robust.Server` [via terminal on macOS/Linux](#running-the-server-on-macos-or-linux)(Note that in bash on linux you mostlikley will have to ``chmod +x Robust.Server`` to be able  to  run it.))
